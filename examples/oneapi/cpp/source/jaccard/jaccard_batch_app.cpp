...
void vertex_similarity_block_processing(const Graph &g,
  ...
  tbb::parallel_for(
          ...
          tbb::parallel_for(
              ...
              [&](const tbb::blocked_range<std::int32_t> &inner_r) {
                for (std::int32_t j = inner_r.begin(); j != inner_r.end(); ++j) {
                  ...
                  auto result_vertex_similarity = preview::vertex_similarity(jaccard_desc, g,
                    processing_blocks[tbb::this_task_arena::current_thread_index()]);

                  nnz += result_vertex_similarity.get_nonzero_coeff_count();
    ...
    /*std::cout << "x16 = " << _x16 << std::endl;
    std::cout << "x16_1 = " << _x16_1 << std::endl;
    std::cout << "x8 = " << _x8 << std::endl;
    std::cout << "x8_1 = " << _x8_1 << std::endl;
    std::cout << "x4 = " << _x4 << std::endl;
    std::cout << "x4_1 = " << _x4_1 << std::endl;
